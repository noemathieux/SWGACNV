...
#' @description This function convert BAM files issued from a Swga sequencing to a specific csv format (chromosome/position/number of reads), rendering it usable by the zscoreProfile function for CNV identification.
#'
...
#' @param output_folder (optionnal) Path of the output folder.
#'
#' @return A csv file for each BAM, usable by the zscoreProfile function.
#'
#' @import Rsamtools
...
pileup_paramsazer <- PileupParam(min_nucleotide_depth = 0, distinguish_nucleotides = FALSE, distinguish_strands = FALSE)
...
for (bam_file in bam_files) {
  ...
  if (!file.exists(bai_file)) {
    ...
    indexBam(bam_file)
    if (!file.exists(bai_file)) {
      stop("ERROR : index file could not be properly generated for ", bam_file)
    }
  ...
  if (!file.exists(bai_file)) {
    stop("Index file could not be found even after creation for :", bam_file)
  }


  pileup_data <- pileup(bam_file, index = bai_file, scanBamParam = scanbam_paramazer, pileupParam = pileup_paramsazer)
